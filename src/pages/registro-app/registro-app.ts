...
export class RegistroAppPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public alertCtrl:AlertController,
              public _rs:RegistroProvider, public loadingCtrl:LoadingController, public _auth:AuthProvider,private camera:Camera,) {

  }

  changePass(data:NgForm) {
    let datos = data.value;

    if (datos.passUser === datos.passConfirm) {
      const loader = this.loadingCtrl.create({ content: "", });
      loader.present();
      this._rs.changePassword(datos.passUser).subscribe( () => {
        loader.dismiss();
  ...
  DataUser(datos:NgForm) {
    let data = datos.value;
    console.log(this.dpiImage);
    if (!this.dpiImage) {
      this.alertCtrl.create({
        title: 'No puedes continuar',
        subTitle: 'Asegurate de haber enviado la foto de tu DPI',
        buttons: ['OK']
      }).present()
    } else {
      let userData = {
        dpi: data.dpiUser,
        pais: data.paisUser,
        direccion: data.direccionUser,
        ciudad: data.ciudadUser
      }

      const loader = this.loadingCtrl.create({ content: "", });
      loader.present();
      this._rs.DatosUser(userData).subscribe(() => {
        loader.dismiss();
        this.slides.lockSwipeToPrev(true);
        this.slides.lockSwipeToNext(false);
        this.slides.slideNext(500);
  ...
  tranUser(datos: NgForm) {
    let data = datos.value.transaccionEmpresa;

    if (!this.rtuImage) {
      this.alertCtrl.create({
        title: 'No puedes continuar',
        subTitle: 'Asegurate de haber enviado la foto de tu RTU',
        buttons: ['OK']
      }).present()
    } else {
      const loader = this.loadingCtrl.create({ content: "", });
      loader.present();
      this._rs.tranUser(data).subscribe(() => {
        loader.dismiss();
        this.slides.lockSwipeToPrev(true);
        this.slides.lockSwipeToNext(false);
        this.slides.slideNext(500);
  ...
  bancoUser(datos: NgForm) {
    let data = datos.value;

    let dataBanco = {
      nombre_banco: data.nBancoEmpresa,
      numero_cuenta: data.noCuentaEmpresa,
      tipo_cuenta: data.tipoCuentaEmpresa,
      nombre_cuenta: data.cuentaEmpresa,
      moneda: data.monedaEmpresa
    }

    const loader = this.loadingCtrl.create({ content: "", });
    loader.present();
    this._rs.BancoUser(dataBanco).subscribe(() => {
      loader.dismiss();
      this.slides.lockSwipeToPrev(true);
      this.slides.lockSwipeToNext(false);
      this.slides.slideNext(500);
  ...
  ionViewDidLoad() {
    let irSlide = this.navParams.get("paso");
    this.slides.paginationType = 'progress';
    // console.log('Slide: ',irSlide);

    if (irSlide == 'password') {
      this.slides.lockSwipes(true);
      this.slides.freeMode = false;
    } else if (irSlide == 'usuario') {
      setTimeout(() => {
        this.slides.lockSwipeToPrev(true);
        this.slides.lockSwipeToNext(false);
